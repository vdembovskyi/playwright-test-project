...
test.describe("My test level 1", () => {
  ...
  test("Check creating an order", async ({ page }) => {
    ...
    await expect(
      page.locator('button[id="remove-sauce-labs-backpack"]'),
    ).toHaveText("Remove");
    // go to the cart
    ...
    await expect(page.locator('span[class="title"]')).toHaveText(
      "Checkout: Overview",
    );
    await page.locator('button[id="finish"]').click();
    // check that order was made
    await expect(page.locator('h2[class="complete-header"]')).toHaveText(
...
*/
  ...
  test("Check add to card goods", async ({ page }) => {
    ...
    // check that order was made
    await expect(cartPage.compleatHeader).toHaveText(
      "Thank you for your order!",
    );
  });
